...
var main = function() {
    ...
    // func to calc the price of the computer
	// Use function expression syntax
	var getPrice = function() {
      var thePrice = 500;  // base price

	  // adjustments for cpu speed
	  if (this.speed === "4GHz") {
		thePrice += 300;
	  }
	  else {
      ...
      // adjustments for hard drive space
	  if (this.hdspace === "1TB") {
		thePrice += 150;
	  }
	  else {
	...
	var Computer = function(speed, hdspace, ram) {
		...
		this.ram = ram;
		this.price = getPrice; // note no parentheses used

	};

	// set prototype.price property to be the getPrice function
	// so a call like computerObject.price() will return the getPrice function value
	Computer.prototype.price = getPrice;

	// Create three Computer instances
